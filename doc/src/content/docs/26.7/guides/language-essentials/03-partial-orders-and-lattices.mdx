...
slug: 26.7/guides/language-essentials/03-partial-orders-and-lattices
---
...
:::danger[Monotonicity of Rules]
In Fixen, rules must be **monotone**, i.e., larger premises must derive larger conclusions. For example, the following rule is not monotone:
...
[31mrel[0m [32mA[0m: [33mNat[0m

[31mrule[0m [33mnotMonotone[0m: [32mA[0m x, [31mif[0m x [35m==[0m [34m0[0m [31m|-[0m [32mA[0m (x [35m+[0m [34m2[0m)
```

It is not monotone because `A 1` invalidates the condition `x == 0` and thus does not cause the rule to fire. However, `A 0`, which is subsumed by `A 1`, validates the condition `x == 0`. Thus, actually, given fact `A 1`, the rule should fire and conclude `A 2`. Fixen does not know this as it keeps track of only the largest facts in the database, so this Fixen program will wrongly not conclude `A 2`.
:::
...
:::caution[Well-Sortedness]
Variables in rules must have a single **sort**, not just a single type. For instance, the following rule is well-typed, but not well-sorted:
...
[31mrel[0m [32mA[0m: [33mNat[0m
[31mrel[0m [32mB[0m: [33mNat'[0m
[31mrel[0m [32mC[0m

[31mrule[0m [33millSorted[0m: [32mA[0m x, [32mB[0m x [31m|-[0m [32mC[0m
```

The `illSorted` rule instantiates the variable `x` with Haskell's `Natural`s. However, it belongs to sort `Nat` (based on `A`) and `Nat'` (based on `B`). Fixen does not know how to unify variables in this scenario.
:::
...
<Downloader url="/files/26.7/guides/language-essentials/03-partial-orders-and-lattices/shortest-paths.tar.gz" sum="ac2123a26811084aa58cc8c3995236b8c893c4c826c674fcd13a227ba9f558bc" />
...
<Steps>
  ...
  2. Populate the files with the following:
     ...
     <Tabs>
       ...
       <TabItem label="Cabal File">
         ```txt title="shortest-paths.cabal"
         cabal-version:      3.0
         name:               shortest-paths
         version:            0.1.0.0
         build-type:         Simple
  ...
  3. Build and run the project!
     ```ansi title="Terminal"
     [35m~/shortest-paths $[0m [31m[1m[4mfixen --output app/ShortestPath.hs fix/ShortestPath.fix[0m
     [35m~/shortest-paths $[0m [31m[1m[4mcabal run[0m
...
<Downloader url="/files/26.7/guides/language-essentials/03-partial-orders-and-lattices/bosp.tar.gz" sum="3a2ea1e5175c3309895d937dd2d5a7d12930002a6da94df9cbd288d9117846ab" />
...
<Steps>
  ...
  3. Build and run the project!
     ```ansi title="Terminal"
     [35m~/bosp $[0m [31m[1m[4mfixen --output app/BOSP.hs fix/BOSP.fix[0m
     [35m~/bosp $[0m [31m[1m[4mcabal run[0m
...
The arguments of sort `A` and `B` are the keys of `R`, while the arguments of sort `C` are the values of `R`. Two facts of `R` are merged as long as their keys match (or they each subsume other facts that have matching keys, obtained via `mlbs`), and their values are merged via the lattice join specified by the declaration of `C`.
:::
