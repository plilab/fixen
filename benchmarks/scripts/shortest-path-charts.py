...
files = os.listdir()
target_file_candidates = sorted([f for f in files if "shortest-path.csv" in f])
...
with open("shortest-path-graph-stats.csv") as f:
...
# Write the chart
plt.savefig(f"benchmarks/charts/shortest-paths.svg", bbox_inches="tight", pad_inches=0)

################################################################################
# table
################################################################################
table_data = sorted(graph_stats_csv[1:], key=lambda x: int(x[2]))
...
# Write the table
with open("benchmarks/charts/shortest-paths-table.tex", "w+") as f:
    f.write("\n".join(tab))
...
speedup_max = round(
    max(no_priorities[i][1] / with_priorities[i][1] for i in range(len(hand))), 1
)

# Write the commands
stats = f"""\\def\\shortestpathnoprioritiesavg{{{np_avg}\\times}}
\\def\\shortestpathwithprioritiesavg{{{wp_avg}\\times}}
\\def\\shortestpathnoprioritiesmax{{{np_max}\\times}}
\\def\\shortestpathwithprioritiesmax{{{wp_max}\\times}}
\\def\\shortestpathspeedupavg{{{speedup_avg}\\times}}
\\def\\shortestpathspeedupmax{{{speedup_max}\\times}}
"""

with open("benchmarks/charts/shortest-paths-statistics.tex", "w+") as f:
    f.write(stats)
