...
module Fixen.Pipeline (pipeline, pipelineFor, pipelineWithSymbolsAndPositions, pipelineWithSymbolsAndPositionsFor) where
...
import Fixen.ModuleSystem (getIncludesWith)
import Fixen.Monad
import Fixen.Parser (parse)
import Fixen.Parser.Cpp qualified as CppParser
...
pipelineFor
  ...
  -> (forall m msg. (MonadIO m, Pretty msg) => Diagnostic msg -> m ())
  -> FixenM (Program, NonEmpty RuleForest, RelationRepresentation, Text)
pipelineFor target code_gen_options file_path contents error_printer = do
  ...
  let parseFile = case target of Haskell -> parse; Cpp -> CppParser.parse
  (program, st) <- run init_env (parseFile file_path (pack contents))
  (program', st') <- run st (getIncludesWith parseFile program)
  (env, st'') <- run st' (solveSymbols program')
  (rt, st''') <- run (env, st'') (getRuleForest program')
  (db, _) <- run st''' getRelationRepresentation
  (t, _) <- run st''' (codeGenFor target code_gen_options rt db program')
...
pipelineWithSymbolsAndPositions = pipelineWithSymbolsAndPositionsFor Haskell

pipelineWithSymbolsAndPositionsFor
  :: Target
  -> CodeGenOptions
  -> FilePath
  -> String
  -> (forall m msg. (MonadIO m, Pretty msg) => Diagnostic msg -> m ())
  -> FixenM (Program, SymbolEnv, PositionEnv, NonEmpty RuleForest, RelationRepresentation, Text)
pipelineWithSymbolsAndPositionsFor target options file_path contents error_printer = do
  let file_map = [(file_path, contents)]
      init_errs = emptyErrors file_map
      init_pos_env = Map.empty
      init_node_id = 0
  ...
  (db, _) <- run st''' getRelationRepresentation
  (t, _) <- run st''' (codeGenFor target options rt db program')
